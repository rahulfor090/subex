...
const footerLinks = {
  ...
  Company: ['About', 'Team', 'Blog', 'Careers', 'Press Kit', 'Contact'],
          ...
          <div key={title} className="col-span-1">
            ...
            <ul className="space-y-4">
              {links.map((link) => {
                ...
                else if (link === 'Cookie Policy') href = '/cookies';
                else if (link === 'Contact') href = '/contact';
                else if (link === 'Careers') href = '/careers';
                else if (link === 'About') href = '/about';
                else if (link === 'Security') href = '/security';
                else if (link === 'Team') href = '/team';
                
                return (
                  <li key={link}>
                    {href !== '#' ? (
                      <Link to={href} className="text-zinc-600 dark:text-zinc-500 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors text-sm block">
                        {link}
                      </Link>
                    ) : (
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex flex-wrap items-center justify-center gap-6">
            {['Privacy', 'Terms', 'Cookies'].map((t) => {
              let href = '#';
              if (t === 'Privacy') href = '/privacy-policy';
              else if (t === 'Terms') href = '/terms';
              else if (t === 'Cookies') href = '/cookies';
              
              return href !== '#' ? (
                <Link key={t} to={href} className="text-zinc-600 dark:text-zinc-600 hover:text-zinc-900 dark:hover:text-zinc-300 transition-colors text-xs font-medium">
                  {t}
                </Link>
              ) : (
